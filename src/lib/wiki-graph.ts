...
export async function buildWikiGraph(
  ...
  for (const [sourceId, nodeData] of nodeMap) {
    for (const targetRaw of nodeData.links) {
      ...
      const targetId = resolveTarget(targetRaw, nodeMap, wikiRoot)
...
function resolveTarget(
  raw: string,
  nodeMap: Map<string, { id: string; path: string }>,
  wikiRoot: string,
): string | null {
  const rawCandidates = linkTargetCandidates(raw)
  for (const candidate of rawCandidates) {
    if (nodeMap.has(candidate)) return candidate
  }

  const normalizedRawCandidates = new Set([...rawCandidates].map(normalizeGraphTarget))
  for (const [id, node] of nodeMap) {
    const relativeToWiki = normalizePath(getRelativePath(node.path, wikiRoot))
    const nodeCandidates = [
      id,
      relativeToWiki,
      `wiki/${relativeToWiki}`,
      getFileName(node.path),
      fileNameToId(getFileName(node.path)),
    ]
    if (nodeCandidates.some((candidate) => normalizedRawCandidates.has(normalizeGraphTarget(candidate)))) {
      return id
    }
  }
...
function linkTargetCandidates(raw: string): Set<string> {
  ...
  if (cleaned.startsWith("wiki/")) candidates.add(cleaned.slice("wiki/".length))

  const fileName = getFileName(cleaned)
  if (fileName) {
    candidates.add(fileName)
    candidates.add(fileNameToId(fileName))
  }

  if (cleaned.endsWith(".md")) candidates.add(cleaned.slice(0, -".md".length))
